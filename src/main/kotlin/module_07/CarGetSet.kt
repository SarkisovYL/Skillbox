...
//  Научим машину ускоряться (accelerate) и замедляться (decelerate) с учетом расхода топлива:

class CarGetSet(val brand: String, val model: String, val color: String = "White") {

    //    Дополнительный конструктор вызывает основной конструктор через слово this с передачей аргументов
    constructor(discriptor: Pair<String, String>, color: String): this (
        brand = discriptor.first,
        model = discriptor.second,
        color = color
    )
//    Тело дополнительных конструкторов описывается в фигурных скобках так же как и тело основной конструктора
    {
